...
export default function BillingScreen() {
  ...
  function handleUpgrade(planId: string, planName: string) {
    ...
    Alert.alert(
      `Upgrade to ${planName}`,
      "Contact us to get Pro access. We'll set up your subscription manually.",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Contact Support",
          onPress: () => {
            Linking.openURL("mailto:support@fpl4flight.io?subject=Pro%20Subscription%20Inquiry&body=I%20would%20like%20to%20upgrade%20to%20Pro.");
          },
        },
      ]
    );
      ...
      >
        ...
        {plans.map((plan) => (
          <View key={plan.id} style={[styles.planCard, plan.popular && styles.planCardPopular]}>
            ...
            {!plan.current && (
              <Text style={styles.contactHint}>Contact us to get Pro access</Text>
            )}
          </View>
        ))}
...
const styles = StyleSheet.create({
  ...
  contactHint: { fontSize: fontSize.xs, color: colors.runway[400], textAlign: "center", marginTop: spacing.xs },
  // Footer
