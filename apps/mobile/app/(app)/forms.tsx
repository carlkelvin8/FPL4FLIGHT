...
export default function FormsScreen() {
  ...
  async function handleCreateForm(templateId: string, templateVersion: number, templateName: string) {
    ...
    } catch (e) { Alert.alert("Error", e instanceof Error ? e.message : "Failed to create form. Check your connection."); }
  ...
  function handleDeleteForm(id: string) {
    Alert.alert("Delete Form", "Are you sure? This cannot be undone.", [
      ...
      { text: "Delete", style: "destructive", onPress: async () => {
        ...
        try { await deleteForm(id); } catch (e) { Alert.alert("Error", e instanceof Error ? e.message : "Failed to delete form."); }
      }},
    ]);
  }
  ...
  function getProgress(formData: Record<string, unknown>, templateId: string): number {
    ...
    const allFields = template.schema.sections?.flatMap((s: { fields?: Array<{ id: string; required?: boolean }> }) => s.fields ?? []) ?? [];
    const requiredFields = allFields.filter((f: { id: string; required?: boolean }) => f.required);
    if (requiredFields.length === 0) return 0;
    const filled = requiredFields.filter((f: { id: string; required?: boolean }) => {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <PressableScale onPress={() => router.push("/(app)/notifications")} haptic accessibilityLabel="View notifications">
      ...
      <View style={styles.filterRow}>
        ...
        {(["all", "draft", "completed"] as FilterOption[]).map((f) => (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            accessibilityLabel={f === "all" ? "Show all forms" : f === "draft" ? "Show drafts" : "Show completed"}
          >
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <PressableScale style={styles.formCard} haptic onPress={() => handleFormPress(item.id)} onLongPress={() => handleDeleteForm(item.id)}>
        ...
        ListEmptyComponent={
          search ? (
            ...
            <EmptyState
              icon="document-text-outline"
              title="No forms yet"
              subtitle="Tap + to start a new form"
      ...
      {/* FAB */}
      <PressableScale style={styles.fab} haptic onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); setShowNewForm(true); }} accessibilityLabel="Create new form">
